...
nofire.rast<-rast(paste(loc.data,"WUIVegetationClassifications/NonFireFootprints_2021-2024_CECSproj.tif",sep=""))
nofire.rast<-check.crs.match(whp.rast,nofire.rast)
yesdist.rast<-rast(paste(loc.data,"WUIVegetationClassifications/ForestDisturbances_2020-2024_CECSproj.tif",sep=""))
yesdist.rast<-check.crs.match(whp.rast,yesdist.rast)

nofire.rast[nofire.rast>=1]<-1
yesdist.rast[yesdist.rast>=1]<-1
...
high.priority.rast<-forest.whp.rast*whp.priority.rast
...
#		for(k in 1:length(policy.target)){
		    ...
		    treat.filt.te.rast<-rasterize(treat.filt.te.vect,whp.rast)
		    treat.nofire.yesdist.rast<-treat.filt.te.rast*nofire.yesdist.rast
		    treat.nofire.yesdist.rast[is.na(treat.nofire.yesdist.rast)]<-0
			...
			#Now need to use rasterized treatments and only places where there were disturbances and no fire
			prepped.zonal.summary.sf$proportion_nofireyesdisttreat<-
				exact_extract(treat.nofire.yesdist.rast,prepped.zonal.summary.sf,fun="mean")
...
#		zonal.treat.agg.sf<-prepped.zonal.summary.sf
...
compiled.data<-prepped.zonal.summary.sf
	plot(compiled.data$proportion_nofireyesdisttreat~compiled.data$highpriority_proportion,
	...
	compiled.data.nona<-compiled.data[!is.na(compiled.data$proportion_nofireyesdisttreat),]
	compiled.data.nona<-compiled.data.nona[!is.na(compiled.data.nona$highpriority_proportion),]

#	print(compiled.data.nona[compiled.data.nona$proportion_nofireyesdisttreat>0.20 & compiled.data.nona$highpriority_proportion>0.60,
#		c("name","Region","HUCareaac","highpriority_proportion","proportion_nofireyesdisttreat")])
#	compiled.data.nona<-as.data.frame(compiled.data.nona[,1:6])

compiled.data.nona$score<-compiled.data.nona$highpriority_proportion*compiled.data.nona$proportion_nofireyesdisttreat

#	write.csv(as.data.frame(compiled.data.nona),paste("HUC12_Treated_HighPriority",boundary.name[i],"_NoFireYesDist.csv"))
...
compiled.data.df$treat_rounded<-round(compiled.data.df$proportion_nofireyesdisttreat,1)
compiled.data.df$combined<-compiled.data.df$proportion_nofireyesdisttreat*compiled.data.df$highpriority_proportion

	write.csv(compiled.data.df,paste("HUC12_Treated_HighPriority_allRegions_ForestNoFireYesDist.csv"))



    plot.title<-paste("HUC12s Treated vs High Priority for ",policy.target, sep="")
    plt<-ggplot(data=compiled.data.df, aes(x=highpriority_proportion,color=Region,y=proportion_nofireyesdisttreat)) +
    ...
    pltnm.b<-paste("HUCProportions_", policy.target[k],"_",datestamp,".png",sep="")
...
best.sf<-zonal.summary.proj.sf[zonal.summary.proj.sf$huc12=="180201510101",] #Glendenning Creek, Sierras
...
# boundary.name<-c("North","South","Central","Sierra")
 ...
 boundary.shape<-c(paste(loc.output,"BestHUC.shp",sep=""))
 boundary.name<-c("Glendenning")


 nice.boundary.name<-c(
 				  "Glendennning Creek - Sierras")
...
prepped.boundary.vect<-read.and.prepare.boundary.vector(boundary.shape,boundary.name,whp.rast)
prepped.boundary.vect$Region<-"Glendenning"

# there aren't any fires in that time in that HUC
#	agg.time<-system.time(agg.fires.region.vect<-intersect.and.aggregate.vectors(
#		prepped.boundary.vect,boundary.name,fire.filt.vect,"Fires","Regions","Region",
#		prepped.boundary.vect,boundary.name))
	    ...
	    print(paste("Time to filter treatments (efficacy): ",round(filter.time[[1]]/60)," minute(s)", sep=""))
			...
			agg.time<-system.time(agg.treatments.region.vect<-intersect.and.aggregate.vectors(
				prepped.boundary.vect,boundary.name,treat.filt.ef.vect,"Treatments","Regions","Region",
				prepped.boundary.vect,boundary.name))
